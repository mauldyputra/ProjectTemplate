...
open class TTGSnackbar: UIView {
    ...
    @objc open dynamic var shadowColor: UIColor = .ttgDefaultShadow
    ...
    fileprivate var XcenterConstraint: NSLayoutConstraint? = nil
...
public extension TTGSnackbar {
    ...
    @objc func show() {
        ...
        if let superView = containerView ?? (UIApplication.shared.delegate?.window ?? nil) ?? UIApplication.shared.keyWindow {
            // Prevent other snackbar to show (only one can exist at once)
            if let currentBar = superView.subviews.filter({ $0.tag == 1669 }) as? [TTGSnackbar] {
                currentBar.forEach({ $0.dismiss() })
            }
            ...
            // Center X constraint
            XcenterConstraint = NSLayoutConstraint.init(
            ...
            XcenterConstraint?.priority = UILayoutPriority(999)
            ...
            XcenterConstraint?.isActive = customContentView != nil
    ...
    fileprivate func showWithAnimation() {
        ...
        case .slideFromLeftToRight:
            ...
            bottomMarginConstraint?.constant = -bottomMargin
            XcenterConstraint?.constant = -superViewWidth
...
public extension TTGSnackbar {
    ...
    fileprivate func dismissAnimated(_ animated: Bool) {
        ...
        case .slideFromTopBackToTop:
            topMarginConstraint?.constant = -snackbarHeight - safeAreaInsets.top - 20
        }
